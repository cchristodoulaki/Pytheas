...
def generate_process_csv_tasks(db_cred, top_level_dir, pytheas_classifier, fold_id, test_keys, max_lines, assume_multi_tables, inject_percent_null, inject_percent_outlier):
    ...
    for file_object in test_files:
        ...
        if  failure is not None or annotations is None:
            ...
            file_counter += 1
            task = (db_cred, file_counter, file_object, pytheas_classifier, fold_id, top_level_dir, max_lines, assume_multi_tables, inject_percent_null, inject_percent_outlier)
            yield task
...
def process_csv_worker(task):
    ...
    db_cred, _, file_object, pytheas_classifier, fold_id, top_level_dir, max_lines, assume_multi_tables, inject_percent_null, inject_percent_outlier = task
    ...
    if inject_percent_null is not None:
        file_dataframe_trimmed = inject_nulls(file_dataframe_trimmed,
                                              annotations,
                                              inject_percent_null)
    if inject_percent_outlier is not None:
        file_dataframe_trimmed = inject_noise(file_dataframe_trimmed,
                                              annotations,
                                              inject_percent_outlier)


    discovered_tables = pytheas_classifier.extract_tables(db_cred, crawl_datafile_key, file_dataframe_trimmed, blank_lines, assume_multi_tables)
...
def eval_pytheas_line_kfold(pytheas_classifier, k_folds, db_cred, top_level_dir, inject_percent_null,
    ...
    for fold_id in folds:
        ...
        pytheas_classifier.train_rules(undersampled_cell_data, undersampled_cell_not_data, undersampled_line_data, undersampled_line_not_data)
        end = timer()

        con = connect(dbname=db_cred.database, user=db_cred.user, host='localhost', password=db_cred.password, port=db_cred.port)
        cur = con.cursor()
        cur.execute(f""" INSERT INTO "{k_folds}fold_cross_validation_pat_model" (fold_id, fuzzy_rules) VALUES (%s, %s)""",
                    (fold_id, Json(pytheas_classifier.fuzzy_rules)))
        con.commit()
        cur.close()
        con.close()

        print(f'\n-training data generated in {timedelta(seconds=end - start)}')
        print(f'Available CPUS:{pytheas.available_cpu_count()}')
        NINPUTS = len(test_keys)
        NPROC = min(num_processors, pytheas.available_cpu_count())
        ...
        pytheas_line_classification_DATA = list(line_prediction.itertuples(index=False, name=None))
        pytheas_cell_classification_DATA = list(cell_prediction.itertuples(index=False, name=None))
        ...
        execute_values(cur, """INSERT INTO pat_line_predictions
                       ...
                       pytheas_line_classification_DATA)
        execute_values(cur, """INSERT INTO pat_cell_predictions
                       ...
                       pytheas_cell_classification_DATA)
        con.commit()
        cur.close()
        con.close()
...
def main():
    ...
    parser.add_argument("-u", "--user", required=True, help="user for the database connection")
    ...
    parser.add_argument("-t", "--top_level_dir", default="./OPEN_DATA_CRAWL_2018", help="path to Open Data Crawl")
    parser.add_argument("-k", "--k_folds", default=10, type=int, help="number of folds in cross-validation")
    parser.add_argument("-m", "--assume_multi_tables", default='True')
    ...
    if args.assume_multi_tables.lower() == 'true':
        assume_multi_tables = True
    elif args.assume_multi_tables.lower() == 'false':
        assume_multi_tables = False
    else:
        sys.exit()
