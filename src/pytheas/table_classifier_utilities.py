...
# def discover_next_table(csv_file, file_offset, table_counter, line_agreements, line_disagreements, blank_lines):
#     # print(f'\nDiscovering table {table_counter}:\n')
#     discovered_table = None
#     csv_file = csv_file.loc[file_offset:]
#     if csv_file.empty:
...
#         predicted_pat_data_lines=[]
#         discovered_table={}
#         discovered_table['first_data_line'] = pat_first_data_line
#         discovered_table['fdl_confidence']["avg_majority_confidence"]=float(first_data_line_combined_data_predictions["avg_confidence"]['confidence'])
#         discovered_table['fdl_confidence']["avg_difference"]=float(first_data_line_combined_data_predictions["avg_confidence"]['difference'])
#         discovered_table['fdl_confidence']["avg_confusion_index"]=float(first_data_line_combined_data_predictions["avg_confidence"]['confusion_index'])
#         discovered_table['fdl_confidence']["softmax"]=float(first_data_line_combined_data_predictions['confidence'])
#         discovered_table['header'] = predicted_pat_header_indexes
...
#         cand_data = csv_file.loc[pat_first_data_line:]
#         aggregation_rows, subheader_scope = predict_subheaders_new(csv_file, cand_data, candidate_pat_sub_headers, blank_lines, predicted_pat_header_indexes, args, rule_weights)

#         predicted_pat_sub_headers = list(subheader_scope.keys())
#         pat_last_data_line = pat_first_data_line
#         pat_last_data_line = predict_last_data_line_top_down(csv_file, pat_first_data_line, line_predictions, args, rule_weights, subheader_scope, aggregation_rows, blank_lines,line_agreements, line_disagreements)

#         predicted_pat_data_lines = list(range(pat_first_data_line, pat_last_data_line+1)) 
...
#         discovered_table['last_data_line'] = pat_last_data_line
        
#         # Last data line predicted
#         ###############################################################################
#         cand_data = csv_file.loc[pat_first_data_line:pat_last_data_line]
#         candidate_pat_sub_headers = list(set(candidate_pat_sub_headers).intersection(set(predicted_pat_data_lines)))
#         aggregation_scope, subheader_scope = predict_subheaders(csv_file, cand_data, candidate_pat_sub_headers, blank_lines, predicted_pat_header_indexes, args, rule_weights)
#         predicted_pat_sub_headers = subheader_scope.keys()
                
#         # print('subheader_scope:')
#         # pp.pprint(subheader_scope)
#         # input(f'\ncandidate pat_sub_headers={predicted_pat_sub_headers}\n')
#         # print('aggregation_scope:')
#         # pp.pprint(aggregation_scope)
#         # input(f'\naggregation rows={list(aggregation_scope.keys())}\n')  
...
# def extract_tables_temp(csv_file, args):
#     discovered_tables= {}
#     blank_lines=[]
#     blank_lines=list(csv_file[csv_file.isnull().all(axis=1)].index)
#     line_agreements,line_disagreements = collect_dataframe_rules(csv_file,args)
#     # print('\n- line_agreements:')
#     # pp.pprint(line_agreements)
#     # print('\n- line_disagreements:')
#     # pp.pprint(line_disagreements)        
#     table_counter=1
#     file_offset=0
#     discovered_table=discover_next_table_temp(csv_file,file_offset,table_counter,line_agreements,line_disagreements,blank_lines)
#     discovered_tables[table_counter]=discovered_table

#     return discovered_tables
    
# def extract_tables(csv_file, args):
#     discovered_tables= {}
#     blank_lines=[]
#     blank_lines=list(csv_file[csv_file.isnull().all(axis=1)].index)
#     line_agreements,line_disagreements = collect_dataframe_rules(csv_file,args)
...
#     table_counter=1
#     file_offset=0
#     discovered_table=discover_next_table(csv_file,file_offset,table_counter,line_agreements,line_disagreements,blank_lines)
#     discovered_tables[table_counter]=discovered_table
#     while discovered_table!=None:
#         discovered_tables[table_counter]=discovered_table
#         table_counter+=1
#         file_offset=discovered_table["last_data_line"]+1
#         # pp.pprint(discovered_table)
#         # # input('Press Enter...')      
#         discovered_table=discover_next_table(csv_file,file_offset,table_counter,line_agreements,line_disagreements,blank_lines)
