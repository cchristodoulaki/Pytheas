...
def avg_and_confidence(frame, label, num_folds):
    avg_performance = (frame.mean() * 100).reindex(['precision', 'recall', 'fmeasure'])
    performance_interval = (frame.sem() * 100).reindex(['precision', 'recall', 'fmeasure'])
    avg_performance.rename(columns=lambda c: c.title(), inplace=True)
    performance_interval.rename(columns=lambda c: c.title(), inplace=True)
    print(f'\n{label} (average over {num_folds} folds)=\n{avg_performance.transpose()}')
    print(f'\nwith 95% confidence interval=\n{performance_interval.transpose()}')
    return avg_performance, performance_interval


def save_performance(db_cred, performance_df, relation_name, fold_id):
    con = connect(dbname=db_cred.database, user=db_cred.user, host='localhost',
                  password=db_cred.password, port=db_cred.port)
    cur = con.cursor()
    if fold_id == 1:
        cur.execute(f"DROP TABLE  IF EXISTS {relation_name}")
        con.commit()

        cur.execute(f"""CREATE TABLE {relation_name} (fold_id integer, measure text) """)
        con.commit()
    ...
    execute_values(cur, f"""INSERT INTO  {relation_name} (fold_id, measure, {','.join(performance_df.columns)}) VALUES %s""",
                   insert_tuples)
    con.commit()
    cur.close()
    con.close()
...
def assign_class(csv_file, discovered_tables, blank_lines, crawl_datafile_key, annotations, fold_id, method='pat'):
